...
function localLoad(){
  state.tasks=JSON.parse(localStorage.getItem('lc_tasks')||'null')||structuredClone(seedTasks);
...
function layout(content,title='Launch Control'){
  const nav=[['dashboard','Dashboard'],['plan','Master Plan'],['mywork','My Work'],['timeline','Timeline'],['decisions','Decisions'],['team','Team']];
...
function renderDashboard(){
  ...
  layout(`<section class="hero"><div><p class="eyebrow">1 November 2026</p><h2>Launch the whole business, not just the website.</h2><p>This board is the single operating view for offer, technology, content, products, ecommerce, marketing, operations and launch readiness.</p></div><div class="pill ${state.backend?'done':'p1'}">${state.backend?'Shared workspace live':'Review mode — backend not connected'}</div></section>
  <section class="status-banner"><div><div class="big">${countdown()} days to go live</div><div class="progress" style="margin-top:12px;background:rgba(255,255,255,.14)"><span style="width:${pct()}%;background:#fff"></span></div><span style="font-size:12px;color:#cbd5e1">${pct()}% of master plan complete</span></div><div class="metric"><strong>${p0.length}</strong><span>P0 open</span></div><div class="metric"><strong>${overdue.length}</strong><span>Overdue</span></div><div class="metric"><strong>${submitted.length}</strong><span>Awaiting review</span></div><div class="metric"><strong>${done.length}</strong><span>Completed</span></div></section>
  <div class="grid two"><section class="card"><div class="card-head"><h3>Launch blockers</h3><button class="btn secondary" data-view="plan">Open master plan</button></div><div class="blocker-list">${blockers.length?blockers.map(t=>`<div class="blocker" data-task="${t.id}"><span class="dot"></span><div><strong>${esc(t.title)}</strong><br><small>${t.id} · ${esc(wsName(t.workstream))} · due ${fmtDate(t.due)}</small></div>${priorityPill(t.priority)}</div>`).join(''):'<div class="empty">No open P0 tasks.</div>'}</div></section>
  <section class="card"><h3>Current phase</h3>${currentPhaseCard()}</section></div>
  <section class="card" style="margin-top:18px"><div class="card-head"><h3>8 workstreams</h3><span class="muted">Click to filter the master plan</span></div><div class="workstream-grid">${workstreams.map(w=>{const ts=state.tasks.filter(t=>t.workstream===w.id);const open=ts.filter(t=>t.status!=='done').length;return `<button class="workstream" data-ws="${w.id}"><span class="count">${open} open</span><h4>${esc(w.name)}</h4><p>${esc(w.description)}</p></button>`}).join('')}</div></section>`, 'Dashboard');
...
function openTaskModal(task){
  const isNew=!task;task=task?structuredClone(task):{id:'',title:'',priority:'P1',workstream:'offer',phase:'core',owner:'Unassigned',due:today(),status:'todo',instructions:'',doneMeans:'',dependencies:[],submissions:[],notes:[]};
  ...
  <div class="field"><label>Priority</label><select name="priority">${priorities.map(p=>`<option ${task.priority===p?'selected':''}>${p}</option>`).join('')}</select></div><div class="field"><label>Status</label><select name="status">${Object.entries(statuses).map(([k,v])=>`<option value="${k}" ${task.status===k?'selected':''}>${v}</option>`).join('')}</select></div>
